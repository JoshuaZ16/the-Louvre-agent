...
def build_official_source_report(
    ...
    evidence_text = " ".join(str(item.get("claim", "")) for item in evidence).lower()
    audits = []
    for claim in claims:
        ...
        audits.append({
            ...
            "matched_evidence": evidence if supported else [],
    ...
    return {
        ...
        "product": product,
        "claims": claims,
        "official_facts": evidence,
        "claim_evidence_audit": audits,
        "source_calls": source_calls,
        "limitations": [] if evidence else ["当前没有可用的官方证据"],
    }
